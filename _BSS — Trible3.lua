...
------------------main_Frame------------------
	...
	local buttonSeeMushroom = CreateButton({Size=UDim2.new(0, 25, 0, 20),Position=UDim2.new(0, 160, 0, 30),
		Text="0",Name="buttonSeeMushroom",Parent=main_Frame,TextSize = 16})
...
local FunZone = {}; do--+++
	...
	checkMonsterInZone = function(zone,monster_name)
	...
	goMonsterInZone = function(zone,monster_name)
		local is,monster = checkMonsterInZone(zone,monster_name)
		if is then
			humanoidRootPart.CFrame = searchCFrame(monster)
			print("GO to Monster",monster_name)
			return monster
		end
	end
end
...
local Auto = {
	...
	digRebut = function(flag)
		repeat
			...
			if flag then wait(600) end
		until (not HumanoidAction.AllStarting) or (not flag)
			or (not AutoFlags.DigBottonDown)
	end,

	humanoidReboot = function()
		while HumanoidAction.AllStarting do
			humanoidRootPart = workspace:WaitForChild(game.Players.LocalPlayer.Name).HumanoidRootPart
			humanoid = game:GetService("Players").LocalPlayer.Character:WaitForChild("Humanoid")
			wait(10)
...
coroutine.wrap(Auto.trackingFunction)()
coroutine.wrap(Auto.humanoidReboot)()
...
local function checkKilled(timeDelay)
	...
	if (Vector3.new(spawnPosHumanoid.x, spawnPosHumanoid.y, spawnPosHumanoid.z) -
		...
		print("Killed")
		coroutine.wrap(Auto.speedRebut)(false)
		--HumanoidState.Speed = 0
		coroutine.wrap(Auto.digRebut)(false)
		--AutoFlags.DigBottonDown = false		
		wait(timeDelay)
		--HumanoidState.Speed = HumanoidState.TmpSpeed
		--coroutine.wrap(WRP.applySpeed)()
...
local function farmKillFunction()
	...
	while AutoFlags.MushroomsBottonDown and buttonSeeMushroom.Visible do
		local mushroom_ = MushroomSearch()
	...
	while HumanoidAction.AllStarting do
		...
		wait(.1)---------------Mushrooms
		if AutoFlags.MushroomsBottonDown and buttonSeeMushroom.Visible then
			AutoFarmMushrooms()
		end
		wait(.1)---------------Ticket
...
------------------Закрытие VioletMainGUI------------------
	buttonDestroy.MouseButton1Down:Connect(function()
		...
		V_I_O_L_E_T_M_A_I_N_Gui:Destroy()
		wait(1)
		ConnectionKey.VirtualUserActive:CaptureController()
		return
	end)
...
spawn(function()------------------settings_Frame---------------
	...
	buttonAutoDig.MouseButton1Down:Connect(function()
		if AutoFlags.DigBottonDown == false then
			...
			coroutine.wrap(WRP.autoDiging)()
			coroutine.wrap(Auto.digRebut)(true)
